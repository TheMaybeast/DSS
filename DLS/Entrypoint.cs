...
namespace DLS
{
    internal class Entrypoint
    {
        ...
        [ConsoleCommand]
        private static void Command_DebugInfo()
        {
            string usedPoolHashes = "";
            foreach (uint hash in UsedPool.Keys.ToList())
                usedPoolHashes = usedPoolHashes + hash + ",";
            usedPoolHashes = usedPoolHashes.Remove(usedPoolHashes.Length - 1);
            Game.LogTrivial("Used Pool: "+UsedPool.Count.ToString()+" ("+usedPoolHashes+")");
            Game.LogTrivial("Available Pool: " + AvailablePool.Count.ToString());
            string _activeVehicles = "";
            foreach (ActiveVehicle aV in activeVehicles)
                _activeVehicles = _activeVehicles + "[" + aV.Vehicle.Handle.ToString() + " | " + aV.PlayerVehicle.ToString() + " ],";
            _activeVehicles = _activeVehicles.Remove(_activeVehicles.Length - 1);
            Game.LogTrivial("Active Vehicles: " + activeVehicles.Count.ToString() + " ("+_activeVehicles+")");
        }
#endif

        [ConsoleCommand]
        private static void Command_RefreshXMLs()
        {
            //Reload DLS Models
            "Reloading: DLS Vehicle Configurations".ToLog();
            DLSModelsDict = Vehicles.GetAllModels();
            "Reloaded: DLS Vehicle Configurations".ToLog();

            //Reload TAgroups
            "Reloading: TAgroups".ToLog();
            tagroups = Vehicles.GetAllTAgroups();
            "Reloaded: TAgroups".ToLog();
        }
    }
}